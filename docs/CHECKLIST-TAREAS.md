...
- [ ] **Nueva etapa – Eliminar "Otros CEUAS" (deprecated).**  
  **Instrucciones:** Sacar toda la funcionalidad de "Otros CEUAS" deprecada: referencias a IsExterno, protocoloexpe externo, "Otros CEUAS", is_external, opciones de protocolo externo en formularios animales/insumos, badges "OTROS CEUAS", modales de ayuda. Limpiar backend, front e i18n. Dejar solo flujo por protocolo estándar. Planificar en una etapa dedicada de limpieza.  
  **Parcial:** En Admin > Usuarios ya se quitó: columna "Otros CEUAS", filtro por Otros CEUAS, ítem de ayuda del modal y toda la lógica asociada en `usuarios.js`. Pendiente: API (UserModel OtrosCeuaCount, InstitucionModel, AdminConfigInstitutionModel, ProtocolModel, AnimalModel, AnimalController, ReactivoModel EsOtrosCeuas), Superadmin instituciones (label "Permitir Otros CEUAS"), config institución (otrosceuas), formulario animales (bloque Otros CEUAS y confirmOtrosCeuas), protocolos.js (showOtrosCeuas / IsExterno).
...
- [ ] **Insumos y Reactivos – badge de tipo de pedido con color (como en Animales).**  
...
- [ ] **Cepa para animales (dropdown obligatorio si existen cepas).**  
...
- [ ] **Configuración de especies: renombrar "Subespecie" a "Categoría de especie" y preparar estructura para cepas.**  
