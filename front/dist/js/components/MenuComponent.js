// front/dist/js/components/MenuComponent.js
...
export async function refreshMenuNotifications() {
    ...
    try {
        ...
        if (res.status === "success") {
            const count = res.data.animales || 0;
            const animalLink = document.querySelector('a[href*="animales.html"] .menu-icon');
            if (!animalLink) return;

            let dot = animalLink.querySelector('.notif-dot');
            if (count > 0) {
                if (!dot) animalLink.insertAdjacentHTML('beforeend', `<div class="notif-dot bg-danger text-white d-flex align-items-center justify-content-center shadow-sm">${count}</div>`);
                else { dot.innerText = count; dot.style.display = 'flex'; }
...
export async function initMenu() {
    ...
    try {
        const [resMenu, resNotif] = await Promise.all([
            ...
            fetch(`/URBE-API-DRIVEN/api/menu/notifications?inst=${instId}`).then(r => r.ok ? r.json() : {data:{animales:0}})
        ]);

        if (resMenu.status === "success" || Array.isArray(resMenu.data)) {
            let ids = resMenu.data || [];
            const notifData = resNotif.data || { animales: 0 };
            
            // Forzar IDs adicionales para admin o superadmin
            if ([1, 2].includes(roleId)) {
                const extras = [4, 9, 10, 202, 203]; // Reactivos, Config, Forms, Contable, Prot. Totales
                extras.forEach(id => { if(!ids.includes(id)) ids.push(id); });
            }
            // Extras para investigador
            if (roleId === 4) {
                const extras = [11, 12, 13, 14]; 
                extras.forEach(id => { if(!ids.includes(id)) ids.push(id); });
            }
            // Perfil siempre presente
            if(!ids.includes(201)) ids.push(201);

            ids.forEach(id => {
                const count = (id === 3) ? (notifData.animales || 0) : 0;
...
style.innerHTML = `
    ...
    .dropdown-menu-gecko { 
        ...
        min-width: 180px; background: #fff; z-index: 1000; border-radius: 12px; margin-top: 10px;
    }
