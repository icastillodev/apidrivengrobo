  ...
  panel__perfil: {
    ...
    blocks: [
      ...
      },
      {
        ...
        html: '<p class="mb-2"><strong>What it is:</strong> opens the <strong>shortcut table</strong> in the app language. The same as pressing <kbd>?</kbd> when focus is <strong>not</strong> in a text field (while typing, <kbd>?</kbd> may type into the field). You can also use <kbd>Alt</kbd>+<kbd>K</kbd> outside those fields.</p><p class="mb-2"><strong>Gecko search:</strong> <kbd>Ctrl</kbd>+<kbd>G</kbd> and <kbd>⌘</kbd>+<kbd>G</kbd> (Mac) open or close the panel even when the cursor is in a field. <kbd>Ctrl</kbd>+<kbd>K</kbd> and <kbd>⌘</kbd>+<kbd>K</kbd> do the same except when you are typing in a field and the panel is closed (so other apps’ shortcuts are not blocked). <kbd>Alt</kbd>+<kbd>G</kbd> toggles search when focus is not in a field that captures keys.</p><p class="mb-2"><strong><kbd>Esc</kbd>:</strong> closes search, open dropdowns, or modal windows in front, depending on context.</p><p class="mb-2"><strong><kbd>Alt</kbd> + letter (navigation):</strong> <kbd>D</kbd> home by role; <kbd>C</kbd> training; <kbd>M</kbd> messages; <kbd>N</kbd> news; <kbd>S</kbd> support; <kbd>O</kbd> profile; <kbd>R</kbd> reservations; <kbd>F</kbd> forms; <kbd>P</kbd> protocols; <kbd>A</kbd> housing; <kbd>Q</kbd> my requests; <kbd>V</kbd> microphone; <kbd>T</kbd> theme; <kbd>Z</kbd> font size; <kbd>L</kbd> top or side menu. Most apply when focus is not in a text field.</p><p class="mb-2"><strong>Sign out:</strong> <kbd>Alt</kbd>+<kbd>Q</kbd> then, within about half a second, <kbd>S</kbd> (sequence <kbd>Q</kbd> then <kbd>S</kbd>).</p><p class="mb-2"><strong>Site administrators:</strong> <kbd>Alt</kbd>+<kbd>X</kbd> then another letter in the same time window: <kbd>P</kbd> protocols, <kbd>A</kbd> housing, <kbd>B</kbd> billing, <kbd>U</kbd> users, <kbd>E</kbd> statistics, <kbd>I</kbd> supplies, <kbd>K</kbd> reagents, <kbd>J</kbd> animals, <kbd>V</kbd> reservations, <kbd>N</kbd> news.</p><p class="mb-0 small text-muted">On phones the keyboard button may be hidden; the table opened here is the live reference and matches what the app implements.</p>',
      },
      {
        id: 'ctrl_layout_menu',
        cat: 'toolbar',
