...
export const CHAPTERS = {
  ...
  panel__perfil: {
    ...
    summary:
      'In-depth guide: how the My profile screen differs from the preferences bar; each control explained on its own (voice, font size, light/dark theme, language, shortcuts, top vs side menu); how settings are saved on the server; and Gecko Search (pill, keyboard, category-based global search and prefixes; voice stays on the menu button).',
    roles:
    ...
    blocks: [
      ...
      },
      {
        ...
        html: '<p class="mb-2"><strong>What it is:</strong> opens the <strong>shortcut table</strong> in the app language. The same as pressing <kbd>?</kbd> when focus is <strong>not</strong> in a text field (while typing, <kbd>?</kbd> may type into the field). You can also use <kbd>Alt</kbd>+<kbd>K</kbd> outside those fields.</p><p class="mb-2"><strong>Gecko search:</strong> <kbd>Ctrl</kbd>+<kbd>G</kbd> and <kbd>⌘</kbd>+<kbd>G</kbd> (Mac) open or close the panel even when the cursor is in a field. <kbd>Ctrl</kbd>+<kbd>K</kbd> and <kbd>⌘</kbd>+<kbd>K</kbd> do the same except when you are typing in a field and the panel is closed (so other apps’ shortcuts are not blocked). <kbd>Alt</kbd>+<kbd>G</kbd> toggles search when focus is not in a field that captures keys.</p><p class="mb-2"><strong><kbd>Esc</kbd>:</strong> closes search, open dropdowns, or modal windows in front, depending on context.</p><p class="mb-2"><strong>Table filter:</strong> <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>F</kbd> (and <kbd>⌘</kbd>+<kbd>Shift</kbd>+<kbd>F</kbd> on Mac) moves focus to the main filter on the visible list.</p><p class="mb-2"><strong><kbd>Alt</kbd> + letter (navigation):</strong> <kbd>D</kbd> home by role; <kbd>C</kbd> training; <kbd>M</kbd> messages; <kbd>N</kbd> forms (new request); <kbd>J</kbd> news; <kbd>S</kbd> support; <kbd>O</kbd> profile; <kbd>R</kbd> reservations; <kbd>F</kbd> forms; <kbd>B</kbd> billing (site admin); <kbd>H</kbd> help for this screen; <kbd>P</kbd> protocols; <kbd>A</kbd> housing; <kbd>Q</kbd> my requests; <kbd>V</kbd> microphone; <kbd>T</kbd> theme; <kbd>Z</kbd> font size; <kbd>L</kbd> top or side menu. Most apply when focus is not in a text field. You can <strong>disable these shortcuts</strong> in the shortcut table (checkbox at the bottom).</p><p class="mb-2"><strong>Sign out:</strong> <kbd>Alt</kbd>+<kbd>Q</kbd> then, within about half a second, <kbd>S</kbd> (sequence <kbd>Q</kbd> then <kbd>S</kbd>).</p><p class="mb-2"><strong>Site administrators:</strong> <kbd>Alt</kbd>+<kbd>X</kbd> then another letter in the same time window: <kbd>P</kbd> protocols, <kbd>A</kbd> housing, <kbd>B</kbd> billing, <kbd>D</kbd> departments (on that screen opens “New department”), <kbd>F</kbd> billing by department, <kbd>U</kbd> users, <kbd>E</kbd> statistics, <kbd>I</kbd> supplies, <kbd>K</kbd> reagents, <kbd>J</kbd> animals, <kbd>V</kbd> reservations, <kbd>N</kbd> news.</p><p class="mb-2"><strong>Modals:</strong> with a modal or alert open, <kbd>C</kbd> confirms or saves when focus is not in a text field; <kbd>Alt</kbd>+letter shortcuts do not navigate while the overlay is open.</p><p class="mb-0 small text-muted">On phones the keyboard button may be hidden; the table opened here is the live reference and matches what the app implements.</p>',
      },
      {
        id: 'ctrl_layout_menu',
        cat: 'toolbar',
      ...
      },
      {
        ...
        html: '<p class="mb-2"><strong>What Gecko Search is:</strong> GROBO’s <strong>central search</strong>: one box to <strong>find records across the app</strong> (requests, protocols, users, housing, supplies, departments, organisations…). Results are <strong>grouped by category</strong> with a clear title and subtitle.</p><p class="mb-2"><strong>The elongated bar with magnifier:</strong> usually shows hint text such as “Search GROBO…”. <strong>Clicking</strong> opens the large panel. With a side menu the bar may appear <strong>floating</strong> at the top; with a top menu it is usually <strong>fixed</strong> in the menu strip.</p><p class="mb-2"><strong>When the panel opens:</strong> the rest of the page <strong>dims</strong> and a <strong>central box</strong> appears with animation. That keeps focus on search without losing the session behind it.</p><p class="mb-2"><strong>Inside the box:</strong> at the top is the <strong>field where you type</strong>. Below, <strong>results by category</strong> appear as you type (with a short delay so the server is not overloaded). If there are no matches, you see a clear message and <strong>prefix</strong> hints (for example <code>request 123</code>, <code>protocol ABC</code>).</p><p class="mb-2"><strong>Shortcuts:</strong> <kbd>Ctrl</kbd>+<kbd>G</kbd> or <kbd>⌘</kbd>+<kbd>G</kbd> open or close this panel from almost any screen, even with focus in a text field. <kbd>Ctrl</kbd>+<kbd>K</kbd> or <kbd>⌘</kbd>+<kbd>K</kbd> do the same except when you are typing in a field and the panel is closed. <kbd>Alt</kbd>+<kbd>G</kbd> toggles the panel when focus is not in a field that captures keys.</p><p class="mb-2"><strong><kbd>Esc</kbd>:</strong> closes the panel and returns to your work screen.</p><p class="mb-2"><strong>Microphone in the panel:</strong> turns dictation on or off using the same voice system as the round menu button.</p><p class="mb-2"><strong><kbd>↓</kbd> and <kbd>↑</kbd> keys:</strong> with the cursor in the search box you can move up and down the list of highlighted results in green.</p><p class="mb-2"><strong><kbd>Enter</kbd>:</strong> opens the <strong>first result</strong> in the list (or the row highlighted with the arrow keys). This panel does not send your phrase to an AI assistant.</p><p class="mb-0 small text-muted"><strong>Voice and AI:</strong> the menu-bar microphone and voice/AI features (if enabled at your site) are separate from the main search listing.</p>',
      },
      {
        id: 'gecko_voz_ia',
        cat: 'modals',
