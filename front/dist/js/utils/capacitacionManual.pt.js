...
export const CHAPTERS = {
  ...
  panel__perfil: {
    ...
    summary:
      'Guia extensa: o que é o ecrã O meu perfil face à barra de preferências; cada botão (voz, letra, tema claro/escuro, idioma, atalhos, menu superior/lateral) explicado à parte; como os dados se guardam no servidor; e Gecko Search (pílula, teclado, pesquisa global por categorias e prefixos; a voz mantém-se no botão do menu).',
    roles:
    ...
    blocks: [
      ...
      },
      {
        ...
        html: '<p class="mb-2"><strong>O que é:</strong> abre a <strong>tabela de atalhos</strong> no idioma da aplicação. Equivale à tecla <kbd>?</kbd> quando o foco <strong>não</strong> está num campo de texto (se estiver a escrever, a <kbd>?</kbd> pode entrar no campo). Também pode usar <kbd>Alt</kbd>+<kbd>K</kbd> fora desses campos.</p><p class="mb-2"><strong>Buscador Gecko:</strong> <kbd>Ctrl</kbd>+<kbd>G</kbd> e <kbd>⌘</kbd>+<kbd>G</kbd> (Mac) abrem ou fecham o painel mesmo com o cursor num campo. <kbd>Ctrl</kbd>+<kbd>K</kbd> e <kbd>⌘</kbd>+<kbd>K</kbd> fazem o mesmo exceto quando escreve num campo com o painel fechado (para não bloquear atalhos de outras aplicações). <kbd>Alt</kbd>+<kbd>G</kbd> alterna o buscador quando o foco não está num campo que capture as teclas.</p><p class="mb-2"><strong><kbd>Esc</kbd>:</strong> fecha o buscador, menus desdobráveis ou janelas emergentes abertas, conforme o que estiver à frente.</p><p class="mb-2"><strong>Filtro de tabela:</strong> <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>F</kbd> (e <kbd>⌘</kbd>+<kbd>Shift</kbd>+<kbd>F</kbd> no Mac) leva o foco ao filtro principal da lista visível.</p><p class="mb-2"><strong><kbd>Alt</kbd> + letra (navegação):</strong> <kbd>D</kbd> início conforme o papel; <kbd>C</kbd> capacitação; <kbd>M</kbd> mensagens; <kbd>N</kbd> formulários (novo pedido); <kbd>J</kbd> notícias; <kbd>S</kbd> suporte; <kbd>O</kbd> perfil; <kbd>R</kbd> reservas; <kbd>F</kbd> formulários; <kbd>B</kbd> faturação (admin sede); <kbd>H</kbd> ajuda deste ecrã; <kbd>P</kbd> protocolos; <kbd>A</kbd> alojamentos; <kbd>Q</kbd> os meus pedidos; <kbd>V</kbd> microfone; <kbd>T</kbd> tema; <kbd>Z</kbd> tamanho de letra; <kbd>L</kbd> menu superior ou lateral. A maior parte só quando o foco não está num campo de texto. Pode <strong>desativar estes atalhos</strong> na tabela de atalhos (caixa no rodapé).</p><p class="mb-2"><strong>Terminar sessão:</strong> <kbd>Alt</kbd>+<kbd>Q</kbd> e, em menos de meio segundo, <kbd>S</kbd> (sequência <kbd>Q</kbd> seguida de <kbd>S</kbd>).</p><p class="mb-2"><strong>Administradores (sede):</strong> <kbd>Alt</kbd>+<kbd>X</kbd> e depois outra letra na mesma janela de tempo: <kbd>P</kbd> protocolos, <kbd>A</kbd> alojamentos, <kbd>B</kbd> faturação, <kbd>D</kbd> departamentos (nesse ecrã abre «Novo departamento»), <kbd>F</kbd> faturação por departamento, <kbd>U</kbd> utilizadores, <kbd>E</kbd> estatísticas, <kbd>I</kbd> insumos, <kbd>K</kbd> reagentes, <kbd>J</kbd> animais, <kbd>V</kbd> reservas, <kbd>N</kbd> notícias.</p><p class="mb-2"><strong>Modais:</strong> com modal ou alerta aberta, <kbd>C</kbd> confirma ou guarda se o foco não estiver num campo de texto; os atalhos <kbd>Alt</kbd>+letra não navegam enquanto o modal estiver aberto.</p><p class="mb-0 small text-muted">Em telemóveis o botão do teclado pode ocultar-se; a tabela que este botão abre é a referência viva e coincide com o que a aplicação implementa.</p>',
      },
      {
        id: 'ctrl_layout_menu',
        cat: 'toolbar',
      ...
      },
      {
        ...
        html: '<p class="mb-2"><strong>O que é o Gecko Search:</strong> o <strong>buscador central</strong> do GROBO: um só quadro para <strong>encontrar registos em todo o sistema</strong> (pedidos, protocolos, utilizadores, alojamentos, insumos, departamentos, organismos…). Os resultados aparecem <strong>agrupados por categoria</strong>, com título e subtítulo legíveis.</p><p class="mb-2"><strong>A barra alongada com lupa:</strong> costuma mostrar um texto guia do tipo «Pesquisar no GROBO…». <strong>Ao clicar</strong> abre-se o painel grande. Com menu lateral a barra pode ver-se <strong>a flutuar</strong> em cima; com menu superior costuma estar <strong>fixa</strong> na faixa do menu.</p><p class="mb-2"><strong>Ao abrir o painel:</strong> o resto da página <strong>escurece</strong> e aparece um <strong>quadro central</strong> com animação. Assim concentra a atenção na pesquisa sem perder a sessão atrás.</p><p class="mb-2"><strong>Dentro do quadro:</strong> em cima está o <strong>campo onde escreve</strong>. Abaixo aparecem os <strong>resultados por categoria</strong> enquanto escreve (com um pequeno atraso para não sobrecarregar o servidor). Se não houver coincidências, verá uma mensagem clara e sugestões de <strong>prefixos</strong> (por exemplo <code>pedido 123</code>, <code>protocolo ABC</code>).</p><p class="mb-2"><strong>Atalhos:</strong> <kbd>Ctrl</kbd>+<kbd>G</kbd> ou <kbd>⌘</kbd>+<kbd>G</kbd> abrem ou fecham este painel em quase qualquer ecrã, mesmo com o foco num campo de texto. <kbd>Ctrl</kbd>+<kbd>K</kbd> ou <kbd>⌘</kbd>+<kbd>K</kbd> fazem o mesmo exceto quando escreve num campo com o painel fechado. <kbd>Alt</kbd>+<kbd>G</kbd> alterna o painel quando o foco não está num campo que capture as teclas.</p><p class="mb-2"><strong><kbd>Esc</kbd>:</strong> fecha o painel e volta ao ecrã de trabalho.</p><p class="mb-2"><strong>Microfone no painel:</strong> liga ou desliga o ditado ligado ao mesmo sistema de voz que o botão redondo do menu.</p><p class="mb-2"><strong>Teclas <kbd>↓</kbd> e <kbd>↑</kbd>:</strong> com o cursor no quadro de pesquisa pode descer e subir pela lista de resultados realçados a verde.</p><p class="mb-2"><strong><kbd>Enter</kbd>:</strong> abre o <strong>primeiro resultado</strong> da lista (ou o realçado com as setas). Este painel não envia a frase a um assistente de IA.</p><p class="mb-0 small text-muted"><strong>Voz e IA:</strong> o microfone da barra do menu e as funções de voz/IA (se a sua sede as tiver ativas) são independentes da listagem principal do buscador.</p>',
      },
      {
        id: 'gecko_voz_ia',
        cat: 'modals',
